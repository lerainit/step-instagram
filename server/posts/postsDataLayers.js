...
const postsSchema = new mongoose.Schema({
    userId: Number,
    posts: [{ id: Number, url: String, hasBackground: Boolean, comments: [{ userIndex: Number, text: String, isVisible: Boolean }], likes: Array,fill:String },]
})
...
export const addLikesData = async (userId, index,user,fill) => {

    let posts = await Posts.findOne({ userId: userId })
    let newPosts = posts.posts[index].likes
    newPosts.push(user) 
    posts.posts[index].likes = newPosts
    posts.posts[index].fill = fill
    posts.save()
    return posts
}
export const removeLikesData = async (userId, index,user,fill) => {
