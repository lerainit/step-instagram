...
const likesReducer = (state = initialValue, action) => {
  ...
  switch (action.type) {
    ...
    case incrementLikes: {
      ...
      let fill =  counter[action.payload.index].fill 
      ...
      fetch(`${process.env.REACT_APP_API_URL_POSTS}/likes/${index}`, {
        ...
        body: JSON.stringify({ user:user.name, userIndex: action.payload.userIndex,userId:userId,fill:fill })
      })
...
const userIndex = likes.findIndex(el => el.name === user.name)
      ...
      counter[action.payload.index].fill = '#95918C'
      let fill =  counter[action.payload.index].fill 
      ...
      fetch(`${process.env.REACT_APP_API_URL_POSTS}/likes/remove/${index}`, {
        ...
        body: JSON.stringify({ user:user, userIndex: action.payload.userIndex,userId:userId,fill:fill })
      })
